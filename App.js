...
function App() {
  ...
  // Function to filter the displayed todos based on the selected filter
  ...
  return (
    <div className="container">
      <h3 align="center">Hello there!! Let's check off some tasks!</h3>
      ...
      <div className="filter-buttons">
        <button id="allbtn" onClick={() => setFilter('All')}>All Tasks</button>
        <button id="donebtn" onClick={() => setFilter('Done')}>Done Tasks</button>
        <button id="todobtn" onClick={() => setFilter('Todo')}>Todo Tasks</button>
      </div>
          ...
          <li key={todo._id}>
            {editingTodo === todo._id ? (
              ...
              <>
                ...
                <div>
                  <button onClick={() => toggleTodoCompletion(todo._id, todo.completed)}>
                    {todo.completed ? 'Uncomplete' : 'Complete'
                  }</button>
                  <button id="edit" onClick={() => startEditing(todo)}>Edit</button>
                  <button id="del" onClick={() => deleteTodo(todo._id)}>Delete</button>
                </div>
              </>
            )}
          </li>
